...
/* 	hs.removeAttribute("searchResult"); */
	...
	if (alTea == null) {
		...
		/* alTea = new TeacherBeanCl().getAllTea(); */
		alTea = new TeacherBeanCl().getTeacherByNameAndDept(teacher,selDepartment,xibie);
	}
...
<!-- <form action="TeaManageServlet?flag=QueryByCriteria" method="post" name="form4"> -->
	<table width="90%" height="269" border="1" cellpadding="0"
		...
		<tr>
			<td colspan="10" align="center">
				<table>
					<tr>
						<td><div>
								教师名:<input name="teacher" type="text" size="10" value="<%=teacher %>"> &nbsp;
								&nbsp;&nbsp;&nbsp;所属系:<select id="selDepartment"
									name="selDepartment">
									<option value="0">所有</option>
									<%
										ArrayList<Department> deptNameArr = new ArrayList<Department>();
										
										deptNameArr = new DepartmentCl().getAllDep(selDepartment);
										
										for (int i = 0; i < deptNameArr.size(); i++) {
									%>
									<option value=<%=deptNameArr.get(i).getDepartment() %>
										<%=(deptNameArr.get(i).getDepartment()).equals(selDepartment) ? "selected": ""%>><%=deptNameArr.get(i).getDepartment()%></option>
									<%
										}
									%>
								</select>
		...
		<tr>
			...
			<td align="center"><%=tb.getXibie() %></td>
			<td align="center"><a
			...
			<td align="center"><%=tb.getType() == 1 ? "系主任" : " 无"%></td>
			<td align="center"><a
				href=<%=path + "/admin/admin.jsp?function=tea_modify&teaId="
						+ tb.getId()%>>修改</a>/<a
		...
		%>
		<tr>
			<td colspan="10" scope="col" align="center" height="30px">
				<%
					if (pageCount > 8) {
 ...
 %> <a
		...
		<tr>
			<td colspan="10" scope="col" align="left" height="30px"><center><a
