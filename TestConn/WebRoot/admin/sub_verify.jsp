...
<script type="text/javascript">
	...
	//筛选院系
	function search() {
		document.form1.action = "SubManageServlet?flag=screeningxibie";
		document.form1.submit();
	}
</script>
<%
	...
	ArrayList<SubjectBean> al = new SubjectBeanCl()
			.getSubByState2(viewType,abDepa);
...
<form name="form1" method="post">
	<table width="80%" align="left" border="1">
		<tr>
			<th colspan="8" height="50px" bgcolor="#E6F3FF"><div
		...
		<tr height="30px" align="center">
			...
			所属系:<select id="selDepartment" name="selDepartment">
				<option value="0">所有</option>
				<%
					ArrayList<String> deptNameArr = new ArrayList<String>();
					
					deptNameArr = new DepartmentCl().getAllDepartmentsByYuanxi(yuanxi);
			...
			<input id="ser" type="button" value="确定" onclick="search()">			
			</td>			
		</tr>
		<tr>
			...
			<td><div align="center">系别</div>
			</td>
			<td><div align="center">题目方向</div>
			</td>
			<td><div align="center">审核状态</div>
			</td>
			<td height="30px" width="7%"><div align="center">选择</td>
		</tr>
		<%
			for (int i = 0; i < (pageNow != pageCount ? pageSize : 1
				...
				TeacherBean tb =new TeacherBeanCl().getTeacherByNameAndYuanxi(sb.getTeacher(), yuanxi);
		...
		<tr>
			...
			<td><div align="center"><%=tb.getXibie()%></div></td>
			...
			<td colspan="7" height="30px"><div align="center">
					...
					<%
							...
							if (pageNow != 1) {
								if (pageCount > 8) {
					%>
					<a href=<%="admin/admin.jsp?function=sub_verify&pageNow=1"%>>[首页]</a>
					<%
						}
					%>
					<a
						href=<%="admin/admin.jsp?function=sub_verify&pageNow="
							+ (pageNow - 1)%>>上一页</a>
					<%
						}
							//页面总数少于8，输出所有页码标签
