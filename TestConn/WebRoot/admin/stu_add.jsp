...
<% if(clsAl.size()==0){%>
	...
	<tr>
		<th>请先添加班级再添加学生，点击<a
			href="admin/admin.jsp?function=../admin/class_manage">添加班级</a>添加班级！</th>
	</tr>
	...
	<table width="65%" height="120" border="1" cellpadding="0"
		...
		<tr>
			<td width="30%" align="center">选择班级：<select name="cls" id="cls"
				onchange="setStuNum()">
					<%
						 clsAl = new ClsBeanCl().getAllCls(yuanxi);
...
<form name="form4" method="post">
	<table border="1" width="65%">
		...
		<tr>
			<td>账&nbsp;&nbsp;号</td><!-- onkeyup="value=value.replace(/[^\d]/g,'') " -->
			<td><input type="text" id="stuId" name="stuId"  onblur="chenkExist()">
	...
	<tr>
		<th colspan="3" scope="col"><div align="left">批量导入学生账号(文件名以xls结尾的excel文件)</div></th>
	</tr>
	<tr>	
		<td colspan="3">
		<form id="file_form" action="UploadServlet" enctype="multipart/form-data"
        method="post">
        <input type="file" name="file" id="file_input" size="50"/> 
        <input type="submit" value="文件上传" id='upFile-btn'>
		</form>
	</tr>
	<% 
		String tishi =null;
	    tishi = (String)request.getAttribute("tishi");
