...
<%
	...
	/* if(selDepartment==null || "".equals(selDepartment)) */ 
	selDepartment=yuanxi;
	System.out.println("院系为："+yuanxi);
	String xibie="0";
	if(teacher==null) teacher="";
	if(key==null) key="";
	if(year==null) year="";
	if(alSub==null){
 	alSub = new SubjectBeanCl().getSubjectByDept(yuanxi);
	}
...
<table width="100%" border="1" >
				...
				<table>
					<tr>
						<td><div>
								...
								&nbsp;&nbsp; 题目方向:<select id="direction"
									name="direction">
									<option value="0">所有</option>
									<%
									ArrayList<Direction> deptNameArr = new ArrayList<Direction>();
										
										deptNameArr = new DirectionCl().getAllDirec(yuanxi);
										
										for (int i = 0; i < deptNameArr.size(); i++) {
									%>
									<option value=<%=deptNameArr.get(i).getDirection() %>
										<%=(deptNameArr.get(i).getDirection()).equals(selDepartment) ? "selected": ""%>><%=deptNameArr.get(i).getDirection()%></option>
									<%
										}
									%>
								</select>
								&nbsp;&nbsp;题目搜索:<input name="key" type="text" size="10" value="<%=key %>"> &nbsp;
								&nbsp;<input name="year" type="hidden" size="10" value="<%=year %>"> &nbsp;
							</div></td>
  ...
  <tr>
    ...
    <td><div align="left"><a href=<%=path+"/common/sub_detail.jsp?id="+sb.getId() %> target="_blank"><%=sb.getTitle() %></a></div></td>
