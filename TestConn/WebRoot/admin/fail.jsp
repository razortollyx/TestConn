...
<%
	...
	ArrayList<StudentBean> stuAl = new StuSubTeaBeanCl().getFailedStu(yuanxi);
	ArrayList<SubjectBean> subAl = new StuSubTeaBeanCl().getFailedSub(yuanxi);
...
<%-- 		if(0==<%=new StageBeanCl().getCurrentStage(yuanxi)%>){
			alert('现在不是调整结果阶段！');
		return;
		} --%>
		if (<%=subAl.size()%> <= 0 || <%=stuAl.size() %><=0){
