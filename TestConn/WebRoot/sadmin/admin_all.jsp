...
<%-- 			<td align="center"><input type="checkbox" name="pick"
			...
			<td align="center"><%=ad.getYuanxi()%><input name="yuanxi" type="hidden"
				value="<%=ad.getYuanxi()%>" /></td>
			<td align="center"><%=ad.getName()%></td>
			<td align="center"><%=ad.getId()%></td>
<%-- 			<td align="center"><%=ad.getSex()%></td> --%>
			<td align="center"><a
				href=<%=path + "/sadmin/admin.jsp?function=tea_modify&teaId="
						+ ad.getId()%>>修改</a>/<a
				onclick="return delConf();"
				href="TeaManageServlet?flag=admin_del&adminId=<%=ad.getId()%>&yuanxi=<%=ad.getYuanxi()%>">删除</a>
			</td>
		</tr>
		<%
			}
		...
		<tr>
			...
			<td colspan="9" scope="col" align="center" height="30px">
				<%
					if (pageCount > 8) {
				%> <a href=<%="sadmin/admin.jsp?function=admin_all&pageNow=1"%>>[首页]</a>
				<%
					}
					if (pageNow != 1) {
				%> <a
				href=<%="sadmin/admin.jsp?function=admin_all&pageNow="
						+ (pageNow - 1)%>>[上一页]</a>
				<%
					}
