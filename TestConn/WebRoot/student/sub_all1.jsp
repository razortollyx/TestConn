...
<form name="form1" action="" method="post">
	...
	if(al==null){
	al = new SubjectBeanCl().getAlternativeSub(Integer.parseInt(tb.getId()));
	}
...
<script type="text/javascript">
	function subm(){
	    ...
	    if(2!=<%=new com.kk.subject.model.StageBeanCl().getCurrentStage(test111)%>){
			alert("非选题阶段!");
			return ;
		} 
	    if(count()<=<%=(new SetBeanCl().getAllinfo(yuanxi).getAlternative()-new StudentSubjectBeanCl().getSelectedAlternativeByStuId(Integer.parseInt(tb.getId())))%>)
	    {
