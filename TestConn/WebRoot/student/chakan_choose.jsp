...
<%
		...
		StudentBean sb = (StudentBean)hs.getAttribute("user");
		 System.out.println(sb.getId()+"xxx");
		ArrayList<StudentSubjectBean> ssbAl = new StudentSubjectBeanCl().getSubAlByStuId(sb.getId());
	   	SubjectBean subBean;
	   int wish = new com.kk.subject.model.StageBeanCl().getCurrentStage(sb.getYuanxi())+1; 	
...
function submit(id){
  ...
  if(2==<%=new com.kk.subject.model.StageBeanCl().getCurrentStage(sb.getYuanxi())+1%>){
...
<table width="100%" border="1">
	...
	<td><div align="center"><%=subBean.getId() %></div></td>
		...
		<td><div align="center">第<%=((StudentSubjectBean)ssbAl.get(i)).getWish()%>志愿</div>
		</td>
		<td><div align="center"><%=arrSSTB.size()%></div>
		</td>
		<td><div align="center"><input id="ser" name="a1" type="text"
	...
	<td><div align="center"><%=subBean.getId() %></div></td>
		...
		<td><input id="ser" name="a" type="text"
				style="background: transparent; border-style: none; width: 100%; height: 100%;"
				value="<%=subBean.getTeacher()%>"
				onclick="showGaryBox('出题人基本信息','student/teainfo.jsp?sub_id=<%=subBean.getId()%>')"></td>
...
<table border="0"  align="center" >
	<td  colspan="7" align="center">
					 (注意：点击导师可查看导师基本信息，点击详情查看可以查看志愿分布信息)
			</td>
	</table>
</form>
</html>
